...
/**
 * 학생 앱이 `ai_recommendations` 에 **직접 쓸 수 있는가**.
 *
 * `false` 인 이유 — 추천값이 서버 산출물이 아니라 클라이언트 순수 함수 스텁
 * (`getStubStudyRecommendation`)의 결과이고, 표에 쓰는 코드도 그 한 곳뿐이며
 * 읽는 코드는 레포 전체에 0건이다. 유료 기능의 산출물 저장소를 사용자가 직접 채울 수
 * 있다는 것 자체가 구멍이라, 2026-08-16 마이그레이션(20260816020000)으로
 * **서버에서 쓰기를 차단**했다. 이 플래그는 그에 맞춰 클라이언트 쓰기를 멈춘다 —
 * 서버만 막으면 저장에 실패하면서 사용자에게 오류만 보인다.
 *
 * ⚠️ 되돌리려면 **추천 생성을 서버로 옮기는 것이 먼저**다(A2). 클라이언트가 만든 값을
 *    다시 쓰게 하면 같은 구멍이다. 플래너 반영(todos 생성)은 학생 자신의 할 일이므로
 *    이 플래그와 무관하게 계속 동작한다.
 */
export const AI_REC_CLIENT_WRITE_ENABLED = false;
