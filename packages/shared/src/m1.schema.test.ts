...
describe("대기 중인 연결 요청의 학생 이름 RPC", () => {
  const migration = readSource(new URL("../../../supabase/migrations/20260815040000_pending_request_student_name.sql", import.meta.url));
  const screen = readSource(new URL("../../../apps/teacher/src/app/m1.tsx", import.meta.url));
  ...
  it("최소 필드만 돌려준다 — 생년월일·목표대학·사진은 없다", () => {
    for (const source of [schema, migration]) {
      ...
      const fn = sliceBetween(
        source,
        "create or replace function pending_connection_requests",
        "comment on function pending_connection_requests"
      );
