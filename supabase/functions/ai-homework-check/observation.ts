...
//
// ⛔ 개정 시도 4종이 **전부 회귀**했다 (2026-08-07 실측 · 사진 3장 · 4분할+10%겹침 · 정답표 16건).
//    **문장을 더하기 전에 반드시 재측정해라. "명백히 좋아 보이는" 수정이 실제로는 나빴다.**
//
//      obs-prompt-1   (기준)                                        정확 13 (재현 12)
//      obs-prompt-2   색중립 + 작성자구분 + 겹침분리 + 위치기준 배제     정확  9
//      obs-prompt-2.1 위 + other_handwritten 을 "마지막 수단"으로       정확 11
//      obs-prompt-3   위치기준 섹션만 제거(절제)                       정확 10
//      obs-prompt-4   **색 2줄만** 추가 (+104토큰)                    정확  9
//
//    · 가장 큰 자책골: other_handwritten 조건을 "문제 번호 바로 옆"으로 좁혔더니, 그 자리가
//      **진짜 채점 표시가 있는 자리**라서 제한이 매칭 규칙으로 읽혔다 → 빨간 동그라미 4개가
//      전부 other_handwritten 이 됐다. 제한은 반드시 **모양 기준 배제**가 먼저여야 한다.
//    · 색 수정 자체는 의도대로 작동했다(mark_color 가 red 일변도에서 black_or_gray 를 쓰기
//      시작). 그런데 **색 2줄만 넣어도 정확이 13 → 9 로 떨어졌다.** 이 프롬프트는 문장을
//      더하는 것 자체에 민감하다.
//    · 연필 별표는 obs-prompt-1 에서도 이미 unlinked 로 잡고 있었다 — 색 편향은 "라벨을
//      잘못 적는" 문제이지 "표시를 못 보는" 문제가 아니었다. 얻는 것보다 대가가 컸다.
//    · 재현성: obs-prompt-1 을 두 번 돌려 13 / 12 (편차 ±1). 위 차이는 잡음이 아니다.
//    요약은 docs/ai-observation-design.md §9.
